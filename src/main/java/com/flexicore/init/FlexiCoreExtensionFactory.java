...
public class FlexiCoreExtensionFactory extends SpringExtensionFactory {
    ...
    public ApplicationContext getApplicationContext(PluginWrapper pluginWrapper) {
        ApplicationContext applicationContext=contextCache.get(pluginWrapper.getPluginId());
        if(applicationContext==null){
            applicationContext=createApplicationContext(pluginWrapper);
            contextCache.put(pluginWrapper.getPluginId(),applicationContext);
        }
    ...
    private ApplicationContext createGenericApplicationContext(PluginWrapper pluginWrapper, ApplicationContext parent) {
        List<Class<? extends com.flexicore.interfaces.Plugin>> beanClasses=pluginManager.getExtensionClasses(com.flexicore.interfaces.Plugin.class,pluginWrapper.getPluginId());
    ...
    private ApplicationContext getParentContext(PluginWrapper pluginWrapper) {
        ...
        List<String> dependencies = pluginWrapper.getDescriptor().getDependencies().parallelStream().map(f -> f.getPluginId()).sorted().collect(Collectors.toList());
